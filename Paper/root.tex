...
As shown in the plot in \ref{F:maxq_gridhole} the QD-learning provides the best Q-function estimation in few steps. Howewer, an inaccurate estimation of the Q-function does not necessarly imply a worse optimal policy estimation. Indeed, as we can see in Figure \ref{F:avg_rew_gridhole}, Q-learning and SpeedyQ-learning have the better performance in terms of average reward per step, while QD-learning outperforms the Double Q-learning and the WQ-learning algorithms. 
...
\begin{minipage}{\textwidth}
  \centering
   \includegraphics[width=.475\textwidth]{./imgs/gridHole/reward.pdf}\label{F:avg_rew_gridhole}
   \includegraphics[width=.475\textwidth]{./imgs/gridHole/maxQ.pdf}	\label{F:maxq_gridhole}
  \end{minipage}
