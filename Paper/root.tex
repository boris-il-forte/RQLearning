...
Despite being suitable also for deterministic problems, as described before \alg is helpful in stochastic environments. In this section, we highlight the main advantages of the adaptation of the learning rate according to the uncertainty of the estimates, in three noisy discrete MDPs. Moreover, we compare the performance of \alg against Q-Learning and some of its variants. These other algorithms are Q-Learning \cite{watkins1992q}, Double Q-Learning \cite{van2010double}, Weighted Q-Learning \cite{d2016estimating} and Speedy Q-Learning \cite{NIPS2011_4251}. They use a single decaying learning rate and generally focus on the bias of the estimate of the action-value, however in this section we show how this is empirically not the main issue in the problem we analyse.

\begin{figure}[t]
\begin{minipage}{\columnwidth}
\centering
  \subfigure[$\alpha = \dfrac{1}{n(s,a)}$\label{F:hasselt_1}]{\includegraphics[scale=.7]{./imgs/gridHasselt/allAlgs1.pdf}}\hspace{-.5cm}
  \subfigure[$\alpha = \dfrac{1}{n(s,a)^{0.8}}$\label{F:hasselt_08}]{\includegraphics[scale=.7]{./imgs/gridHasselt/allAlgs08.pdf}}
\end{minipage}
...
\begin{minipage}{\columnwidth}
\centering
  \subfigure[$\alpha = \dfrac{1}{n(s,a)}$\label{F:hasselt_1}]{\includegraphics[scale=.7]{./imgs/gridHasselt/QDecs1.pdf}}\hspace{-.5cm}
  \subfigure[$\alpha = \dfrac{1}{n(s,a)^{0.8}}$\label{F:hasselt_08}]{\includegraphics[scale=.7]{./imgs/gridHasselt/QDecs08.pdf}}
\end{minipage}
...
This environment is proposed in \cite{van2010double} and consists in a $3 \times 3$ grid with the initial position in the lower-left cell and the goal state in the upper-right cell. Each action performed in a non-goal state obtains a reward $-12$ and $10$ with equal probability. In the goal state, every action obtains a reward of $5$ and terminates the episode. The discount factor is $\gamma = 0.95$. The policy is $\varepsilon$-greedy with $\varepsilon = \frac{1}{\sqrt{n(s)}}$, where $n(s)$ is the number of  visits of the state $s$. We analyse the performance of \alg with different choices of learning rates, on the other hand for all the other algorithms we use a decaying learning rate $\alpha(s, a) = \frac{1}{n(s, a)^{k}}$ where $n(s, a)$ and $k$ are respectively the number of updates for each action $a$ in state $s$\footnote{Double Q-Learning splits $n(s,a)$ in $n_A(s,a)$ and $n_B(s,a)$ that are the number of updates for each action $a$ in state $s$ in the two Q-Tables.} and a coefficient to tune the rate of decay. In \alg experiments generally $\eta = 1$, unless otherwise specified.

The optimal average reward per step is $0.2$ and the maximum action-value function of the initial state is $5\gamma^4 - \sum_{k=0}^3 \gamma^k \approx 0.36$. Figure \ref{F:hasselt_all} shows the mean reward per step and the approximation of the maximum action-value in the initial state computed by the other algorithms and \alg with the same learning rate $\alpha$, but with the separated variance-dependent learning rate for the action-value estimate, that is the most effective setting of \alg that we tried in this problem. Note how the performance of \alg w.r.t. reward are the best one and, moreover, how the estimate of the action-value is also the best one. This interestingly suggests that excellent performance can be reached ignoring the problem of the bias of the estimate described in \cite{van2010double}. Figure \ref{F:hasselt_QDecs} shows the same \alg performance of the previous figure (setting ``QDecBeta'') in comparison with other less effective \alg settings. Using a variance-dependent learning rate $\alpha$ with $\eta = 100$ for the estimate of the reward function and a variance-dependent learning rate $\beta$ for the estimate of the action-value function (setting ``QDecAlphaBeta''), gives similar result of setting ``QDecBeta''. Setting ``QDecDelta'' uses the learning rate $\delta$ described in Equation \ref{eq:delta_eq} and gives poor results due to the high stochasticity of the problem.
...
\end{minipage}
\caption{...}
  \label{F:double_chain}
\end{figure*}

\begin{figure*}[t]
\begin{minipage}{\textwidth}
\centering
  \subfigure[$\alpha = \dfrac{1}{n(s,a)}$\label{F:lrs_1}]{\includegraphics[scale=.7]{./imgs/doubleChain/lrs1-1.pdf}}
  \subfigure[$\alpha = \dfrac{1}{n(s,a)^{0.51}}$\label{F:lrs_51}]{\includegraphics[scale=.7]{./imgs/doubleChain/lrs1-51.pdf}}
  \subfigure[$\alpha = \dfrac{1}{n(s,a)}$\label{F:lrs_1}]{\includegraphics[scale=.7]{./imgs/doubleChain/lrs5-1.pdf}}
  \subfigure[$\alpha = \dfrac{1}{n(s,a)^{0.51}}$\label{F:lrs_51}]{\includegraphics[scale=.7]{./imgs/doubleChain/lrs5-51.pdf}}
\end{minipage}
\caption{...}
  \label{F:double_chain}
\end{figure*}
